...
def convert_h5ad_to_df(adata):
    if type(adata.X)==np.ndarray:
        ...
        X=np.array(adata.X)
    elif type(adata.X)==scipy.sparse.csr.csr_matrix or type(adata.X)==scipy.sparse.coo.coo_matrix:
        X=np.array(adata.X.todense())
    else:
        sys.exit("adata.X is of type {} which is not implemented.".format(type(adata.X)))
    genes = adata.var_names.tolist()
    df = pd.DataFrame(X, columns=genes, index=adata.obs["cell.type"].tolist())
    if "subject" in adata.obs.columns:
        subjects = pd.DataFrame(adata.obs.subject.tolist(), index=list(range(df.shape[0])))
