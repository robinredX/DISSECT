...
def explain(config):
    ...
    sample_names = pd.read_table(
        os.path.join(data_path, "sample_names.txt"), index_col=0
    )
    celltypes = pd.read_table(
        os.path.join(data_path, "celltypes.txt"), index_col=0
    ).index.tolist()
    genes = pd.read_table(
        os.path.join(data_path, "genes.txt"), index_col=0
    ).index.tolist()
    ...
    df_train = pd.DataFrame(
        X_train_norm,
        columns=genes,
        index=["sample_{}".format(i) for i in range(X_train.shape[0])],
    )
    df_test = pd.DataFrame(
        X_test_norm,
        columns=genes,
        index=["sample_{}".format(i) for i in range(X_test.shape[0])],
    )
    ...
    adata_train = AnnData(
        df_train, obs=pd.DataFrame(y_train, index=df_train.index, columns=celltypes)
    )
    ...
    for model_no in [0]:  # range(len(config["seeds"])):
        model_folder = os.path.join(
            config["experiment_folder"], "model_{}".format(model_no)
        )
    ...
    print(
        "Shapley values of each celltype and each sample are saved at {}".format(
            os.path.join(shap_dir, "shap_values_all.npy")
        )
    )
    for i in range(len(celltypes)):
        celltype = celltypes[i]
