...
sys.path.append("/Users/robin/Desktop/dissect_src")
from configs.glob_config import config
import warnings
...
result_file1 = os.path.join(config["experiment_folder"], "results/datasets/DSTG_results.txt")
#result_file2 = '{}/true_output.csv'.format(FLAGS.result)
np.savetxt(result_file1, ab[pred_mask], delimiter=',')

celltypes = pd.read_csv("Datadir/Real_Label2.csv", index_col=None).columns.tolist()
df_output = pd.DataFrame(ab[pred_mask], columns=celltypes)
df_output.to_csv(result_file1, sep="\t")
#np.savetxt(result_file1, ab[pred_mask], delimiter=',')
