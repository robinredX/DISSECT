...
def run_dissect_expr(config):
    ...
    if not os.path.exists(ens_path):
        if not os.path.exists(zero_path):
            sys.exit("Could not find prediction proportions for the real data. Did you run dissect.py. They are in experiment_folder specified in the config file and have file name dissect_fraczions_[model_number].txt). ")
        else:
    ...
    for celltype in X_celltypes.keys():
        X_celltypes[celltype] = np.concatenate([X_celltypes[celltype], 
                                                np.array(data.obs[celltype]).reshape((data.shape[0],1))], 1)
    ...
    for celltype in tmp.keys():
        tmp[celltype] = pd.DataFrame(tmp[celltype], index=["s_{}".format(i) for i in \
                                                        range(tmp[celltype].shape[0])])

    tmp_ls = []
    print("Preparing training datasets.")
    for i in tqdm(range(X_celltypes[list(X_celltypes.keys())[0]].shape[0])):
        tmp_ls.append(np.concatenate([X_celltypes[celltype][i,:].reshape((1,X_celltypes[celltype].shape[1])) \
                                    for celltype in X_celltypes.keys()],
                            axis=0))
    ...
    class Decoder(layers.Layer):
        ...
        def __init__(self,
            ...
            self.dense_output = layers.Dense(original_dim-labels.shape[1], activation="relu")
    ...
    class VariationalAutoEncoder(tf.keras.Model):
        ...
        def __init__(self,
            ...
            self.decoder = Decoder(original_dim, intermediate_dim1=intermediate_dim3,
                                    intermediate_dim2=intermediate_dim2, intermediate_dim3=intermediate_dim1)
    ...
    data = tf.data.Dataset.from_tensor_slices((X_sim_input, X_sim_gt, 
                                            X_sim_input[:,X_sim_gt.shape[1]:], fract))
    # data = data.shuffle(1000).repeat().batch(batch_size=batch_size)
    data = data.repeat().batch(batch_size=batch_size)
    data_iter = iter(data)

    data1 = tf.data.Dataset.from_tensor_slices((X_real_input,
                                            X_real_input[:,X_sim_gt.shape[1]:], fract_real))
    ...
    mse_loss_fn = tf.keras.losses.MeanSquaredError(reduction=tf.keras.losses.Reduction.SUM)
    ...
    for epoch in pbar:
        ...
        for step in range(n_steps):
            ...
            with tf.GradientTape() as tape:
                reconstructed, kld = vae(x_train1[:,:x_train2.shape[1]], labels=labels)
                reconstructed_r, _ = vae(x_train1[:,:x_train2.shape[1]], labels=labels)
                recon_loss = mse_loss_fn(x_train2, reconstructed)
                # fractions_loss = mse_loss_fn(true_fractions, fractions)
                x_train1_reconstructed = reconstructed*tf.expand_dims(true_fractions,1)
                x_train1_reconstructed_r = reconstructed_r*tf.expand_dims(true_fractions_r,1)
                        
                fractions_loss = mse_loss_fn(x_train1[:,:x_train2.shape[1]]/labels.shape[1], 
                                                x_train1_reconstructed)
                fractions_loss_r = mse_loss_fn(x_train1_r[:,:x_train2.shape[1]]/labels.shape[1], 
                                                x_train1_reconstructed_r)
                #kld = sum(vae.losses)
                loss = recon_loss + 0.01*kld + 0.1*(fractions_loss+fractions_loss_r)/2
                total_loss_kl +=kld
    ...
    est, _ = vae(X_real_input[:,:x_train2.shape[1]], labels=X_real_input[:,X_sim_gt.shape[1]:])
    est = est*max_val
    ...
    print("Estimated gene expression per cell type is saved at {} and is included in the layers attribute of the anndata object.".format())
