...
def run_dissect(config):
    ...
    for seed in config["seeds"]:
        ...
        dataset = (
            ...
            .repeat()
            .batch(batch_size=config["network_params"]["batch_size"])
        )
        ...
        model = network(
            config["network_params"], n_celltypes, n_features, training=True
        )
        ...
        optimizer = tf.keras.optimizers.Adam(
            learning_rate=config["network_params"]["lr"]
        )
        ...
        for i in pbar:
            ...
            with tf.GradientTape() as tape:
                ...
                if step < 2000:
                    ...
                    if (
                        config["sig_matrix"]
                        or config["test_dataset_type"] == "spatial_sparse"
                    ):
                        lambda_ = 0.15
            ...
            if evaluate:
                ...
                pbar.set_description(
                    "step: %d| Losses - total_loss: %.4f | reg_loss: %.4f | cons_loss: %.4f | r: %.4f | Avg r: %.4f"
                    % (step, loss_, reg_loss, cons_loss, r, avgr)
                )
            step += 1
            pbar.set_description(
                "step: %d| Losses - total_loss: %.4f | reg_loss: %.4f | cons_loss: %.4f"
                % (step, loss_, reg_loss, cons_loss)
            )
