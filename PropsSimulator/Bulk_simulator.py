...
class Simulate(object):
    def __init__(self, config):
        ...
        self.celltypes = np.sort(
            np.array(self.sc_adata.obs[config["celltype_col"]].unique())
        )
    ...
    def generate_props(self):
        ...
        self.props_complete = np.random.dirichlet(
            self.config["concentration"], self.n_complete
        ).astype(np.float32)
        ...
        plt.savefig(
            os.path.join(config["experiment_folder"], "boxplot_props_complete.pdf")
        )
        fig = plt.figure()
        ...
        plt.savefig(
            os.path.join(config["experiment_folder"], "boxplot_ncells_complete.pdf")
        )
        ...
        plt.title("Proportions of cell-types in generated samples")
        plt.savefig(
            os.path.join(config["experiment_folder"], "boxplot_props_sparse.pdf")
        )
        fig = plt.figure()
        ...
        plt.title("Counts of cell-types in generated samples")
        plt.savefig(
            os.path.join(config["experiment_folder"], "boxplot_ncells_sparse.pdf")
        )
    ...
    def preprocess(self):
        ...
        self.sc_adata = self.sc_adata[
            self.sc_adata.obs.pct_counts_mt < self.config["filter"]["mt_cutoff"], :
        ]
    ...
    def simulate(self, save=True):
        ...
        for celltype in self.celltypes:
            adatas[celltype] = np.array(
                self.sc_adata[
                    self.sc_adata.obs[self.config["celltype_col"]] == celltype
                ].X
            )
        self.sc_adata = None
        ...
        if save:
            ...
            plt.savefig(
                os.path.join(
                    config["experiment_folder"], "scatterplot_pca_simulated.pdf"
                )
            )
    ...
    def simulate_per_batch(self, save=True):
        ...
        for batch in self.batches:
            self.sc_adata = adata_orig[
                adata_orig.obs[self.config["batch_col"]] == batch
            ]
